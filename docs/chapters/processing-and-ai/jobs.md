...
For BASH and Python jobs it is often useful to call the NORA backend CLI `nora` directly from within the job. This gives access to project-aware file selection, tagging, metadata export, output-path computation, and other backend functions described in [Administration Backend](../installation-and-administration/administration-backend.md).

![image-1601211287963.png](../../assets/images/chapters/processing-and-ai/jobs-overview.png)
...
This adds a tag to the file selected for processing. If the project context is already available through `DPXproject`, `-p MYPROJECT` can usually be omitted. For selector syntax and further examples, see [Administration Backend](../installation-and-administration/administration-backend.md).
